...
class _LoginScreenState extends ConsumerState<LoginScreen> {
  ...
  Future<void> _submitMockLogin({
    ...
    final valid = username == 'demo' && password == 'demo';
    if (!valid) {
      if (!mounted) {
        return;
      }
      setState(() => _errorText = 'Modo mock: usa demo / demo.');
      return;
    }
  ...
  _SessionPayload _buildSessionPayload({
    ...
    if (token == null || token.trim().isEmpty) {
      throw StateError('El backend no devolvió access_token/token para la sesión móvil.');
    }

    return _SessionPayload(
      token: token,
      expiresAt: _extractExpiresAt(payload) ?? DateTime.now().toUtc().add(_fallbackSessionTtl),
    );
  }
  ...
  DateTime? _extractExpiresAt(Map<String, dynamic>? payload) {
    for (final map in _candidateMaps(payload)) {
      ...
      final ttlSecondsValue = map['ttl_seconds'] ?? map['ttlSeconds'];
      final ttlSeconds = _parseDurationFromSeconds(ttlSecondsValue);
      if (ttlSeconds != null) {
        return DateTime.now().toUtc().add(ttlSeconds);
      }
  ...
  List<Map<String, dynamic>> _candidateMaps(Map<String, dynamic>? payload) {
    ...
    for (final key in const <String>['data', 'result', 'session', 'payload', 'user']) {
